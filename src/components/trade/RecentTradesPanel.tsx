...
/** 오더북 아래 — 바이낸스 선물 최근 체결 (호가보다 낮은 비중) */
const RecentTradesPanel: React.FC = () => {
  ...
  return (
    <div className="flex-[1] min-h-0 max-h-[48%] flex flex-col bg-[#0b0e11] overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scroll">
        {visible.length === 0 ? (
          <div className="px-2.5 py-2 text-center text-xs text-[#848e9c]">
            체결 수신 중...
          </div>
        ) : (
          <div className="px-2.5 py-0.5">
            {visible.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className="grid grid-cols-3 gap-1 text-xs leading-5 tabular-nums"
                >
