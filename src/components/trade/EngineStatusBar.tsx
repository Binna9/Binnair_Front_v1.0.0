...
const EngineStatusBar: React.FC = () => {
  const { run, runs, loading } = useEngineRun();
  ...
  const otherPaused = runs.filter(
    (r) => r.run_id !== run.run_id && r.status === 'paused'
  ).length;
  ...
  return (
    <div className="flex-shrink-0 flex items-center gap-4 px-4 py-2.5 text-sm border-b border-[#2b3139] overflow-x-auto custom-scroll">
      <span className="flex items-center gap-2 font-semibold text-[#eaecef] whitespace-nowrap">
        <span
          className={`w-2.5 h-2.5 rounded-full ${STATUS_DOT[run.status] ?? STATUS_DOT.stopped}`}
        />
      ...
      {runs.length > 1 && (
        <span className="text-[#5e6673] whitespace-nowrap">
          세션 {runs.length}개
          {otherPaused > 0 ? ` · paused ${otherPaused}` : ''}
        </span>
      )}
      {run.paper_mode && (
